...
function getFilename(longname) {
    ...
    // --------------------------------------------------------------------
    // 2017-10-13
    // This modification fixes an issue where processing FeatureLayer and
    // WMTSLayer only would create a duplicate unique filename for WTMSLayer
    // when using node versions > v8.2.1.
    // hasOwnProp.call would initially be false, then true for 9 times and
    // then be false again, when module:esri/layers/WMTSLayer was clearly
    // in the dict longnameToUrl.
    // This fix checks the array index for the module name as a fallback to
    // prevent the unknown case when it reports as false when it does exist.
    //
    // hasOwnProp.call(longnameToUrl, module:esri/layers/WMTSLayer):301: false
    // makeUniqueFilename(filename: module-esri-layers-WMTSLayer, str: module:esri/layers/WMTSLayer)
    // getUniqueFilename(str: module:esri/layers/WMTSLayer) - return module-esri-layers-WMTSLayer.html
    // --------------------------------------------------------------------
    var hasKey = Object.keys(longnameToUrl).indexOf(longname) !== -1;
    if ( hasOwnProp.call(longnameToUrl, longname) || hasKey ) {
...
var parseType = exports.parseType = function(longname) {
    var err;
...
function cleanseStringify(parsedType, stringifiedValue){
  // --------------------------------------------------------------------
  // Regular Expression to determine how many nested Arrays there are in
  // the data type signature.
  // /(Array\.?(<|&lt;))+
  // --------------------------------------------------------------------
  var numberOfArraysRegEx = /[Aa]rray\.?(<|&lt;)/g;

  // --------------------------------------------------------------------
  // Regular Expression to match the inner signature of an array of some data type.
  // e.g., Promise<Graphic[]>
  // module:esri/geometry/Geometry|Array<module:esri/geometry/Geometry>
  // module:esri/geometry/Geometry|Array<Array<module:esri/geometry/Geometry>>
  // module:esri/Graphic|Array<module:esri/Graphic>|Number|Array<Number>
  // Array<module:esri/Graphic>|Array<Object>
  //
  // Used to find/replace the inner part of the stringified parsed type
  // whose value is an array of something
  // --------------------------------------------------------------------
  var innerArrayOfTypesRegEx = /Array\.(<|&lt;)\((<[^>]*>[:/\w]*<[^>]*>\|?)*\)>/im;
  var innerSingleDimArrayRegEx = new RegExp("([aA]rray\\.?(<|&lt;))+<[^>]*>[-~:\*\\w\/]+<[^>]*>>", "g");
  var innerSingleDimArrayNoGlobalRegEx = new RegExp("([aA]rray\\.?(<|&lt;))+<[^>]*>[-~:\*\\w\/]+<[^>]*>>");
  var innerMultiDimArrayRegExRaw = "([aA]rray\\.?(<|&lt;))+<[^>]*>[-~:\*\\w\/]+<[^>]*>>{COUNT}";

  var typeExpression = parsedType.typeExpression;

  // --------------------------------------------------------------------
  // Make sure the signature matches an Array, Promise or Foo[]
  // --------------------------------------------------------------------
  if (/(array|promise|\w*\[\])/i.test(typeExpression)) {

    if (numberOfArraysRegEx.test(stringifiedValue)) {
      var numberOfMatches = stringifiedValue.match(numberOfArraysRegEx).length;
      // complete the Regular Expression by replacing COUNT with the real number
      var innerArrayRegEx = RegExp(innerMultiDimArrayRegExRaw.replace("COUNT", numberOfMatches));
      // capture the single or multi-dimensional array including the html anchor
      var multiDimensionalArray = stringifiedValue.match(innerArrayRegEx);

      if (multiDimensionalArray) {
        // captured the opening and closing element of the anchor tag
        var anchorPartsMatch = multiDimensionalArray[0].match(/<[^>]*>/g);
        // single: [], or multi: [][][] depending on the complexity
        var arrayBracketSignature = stringifiedValue.match(numberOfArraysRegEx).map(() =>{
          return "[]"
        });
        // the completed html anchor <a href="foo">Bar[]</a> link
        // instead of Array.<<a href="foo">Bar</a>> link
        let arrayPrefixSignature = multiDimensionalArray[0].match(/(\w*|\*)(?=<\/a>)/i);
        if (arrayPrefixSignature && /^[a-z]/.test(arrayPrefixSignature[0])) {
          var text = arrayPrefixSignature[0];
          arrayPrefixSignature[0] = text[0].toUpperCase() + text.substring(1);
        }
        var innerAnchor = anchorPartsMatch[0] + arrayPrefixSignature[0] + arrayBracketSignature.join("") + anchorPartsMatch[1];

        stringifiedValue = stringifiedValue.replace(innerArrayRegEx, innerAnchor);
      }
      else {
        // capture the single array including the html anchor
        var singleDimensionalArray = stringifiedValue.match(innerSingleDimArrayRegEx);
        if (singleDimensionalArray) {
          singleDimensionalArray.forEach(function (singleDimArray){
            // captured the opening and closing element of the anchor tag
            var anchorPartsMatch = singleDimArray.match(/<[^>]*>/g);
            // single: [], or multi: [][][] depending on the complexity
            var arrayBracketSignature = "[]";
            // the completed html anchor <a href="foo">Bar[]</a> link
            // instead of Array.<<a href="foo">Bar</a>> link
            let arrayPrefixSignature = singleDimArray.match(/(\w*|\*)(?=<\/a>)/i);
            if (arrayPrefixSignature && /^[a-z]/.test(arrayPrefixSignature[0])) {
              var text = arrayPrefixSignature[0];
              arrayPrefixSignature[0] = text[0].toUpperCase() + text.substring(1);
            }
            var innerAnchor = anchorPartsMatch[0] + arrayPrefixSignature[0] + arrayBracketSignature + anchorPartsMatch[1];

            stringifiedValue = stringifiedValue.replace(innerSingleDimArrayNoGlobalRegEx, innerAnchor);
          });
        }
        else if(innerArrayOfTypesRegEx.test(stringifiedValue)){
          stringifiedValue = stringifiedValue.replace(/Array\.(<|&lt;)\((?=(<[^>]*>\w*<[^>]*>\|?)*)/gi, "");
          stringifiedValue = stringifiedValue.replace(/\)>$/, "");
          var innerLinkMatch = stringifiedValue.match(/<[^>]*>\w*<\/a>/g);
          innerLinkMatch.forEach(function (linkMatch){
            var linkText = linkMatch.match(/\w*(?=<\/a>)/);
            var patchedLinkMatch = linkMatch.replace(new RegExp(linkText + "<\/a>"), linkText[0] + "[]</a>");
            stringifiedValue = stringifiedValue.replace(linkMatch, patchedLinkMatch);
          });
        }
      }
    }
  ...
  if (/object\.?(&lt;|<)string, string>/i.test(typeExpression)) {
    stringifiedValue = "<a href=\"https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String\">String[]</a>";
  }

  // --------------------------------------------------------------------
  // Replace Foo.<Bar> with Foo<Bar> or Foo.&lt;Bar> with Foo&lt;Bar>
  // --------------------------------------------------------------------
  if (/\.(&lt;|<)/g.test(stringifiedValue)) {
    var cleansedStringifiedType = stringifiedValue.replace(/\.(&lt;|<)/g, "&lt;");
    stringifiedValue = cleansedStringifiedType;
  }
  ...
  if (/(object|function|number|string|boolean)<\/a>/g.test(stringifiedValue)) {
    // find the match
    ["object", "function", "number", "string", "boolean"].forEach(function (type){
      var typeRegEx = new RegExp(type + "<\/a>", "i");
      var linkTextMatch = stringifiedValue.match(typeRegEx);
      if (linkTextMatch) {
        var text = linkTextMatch[0];
        var linkTextFixed = text[0].toUpperCase() + text.substring(1);
        stringifiedValue = stringifiedValue.replace(typeRegEx, linkTextFixed + "</a>");
      }
    });
  }
 ...
 */
var stringifyType = exports.stringifyType = function(parsedType, cssClass, stringifyLinkMap) {
