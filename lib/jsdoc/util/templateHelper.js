...
function stringifyType(parsedType, cssClass, stringifyLinkMap) {
    ...
    if (stringifiedType.match(/esri\/(\w*\/)*/m)){
        ...
        // --------------------------------------------------------------------
        stringifiedType = stringifiedType.replace(/module:(\w*\/)*/gm, "");

        // --------------------------------------------------------------------
        // Given an anchor element such as:
        // <a href="module-esri-core-Collection.html">module:esri/core/Collection</a>.&lt;<a href="module-esri-layers-Layer.html">module:esri/layers/Layer</a>>
        // Look for the Foo.&lt;Bar> link, aka Foo.<Bar>
        //
        // .&lt;<a href="module-esri-layers-Layer.html">module:esri/layers/Layer</a>>
        //
        // searchText.match(/\.&lt;<[^>]*>[a-zA-Z0-9:\\/]*<[^>]*>>/i)
        //
        // The first Regular Expression will clean up the period between the Promise and <
        // such as Promise.<FOO>
        // Promise.<Array.<module:esri/Graphic>> or Promise.<Array.<Array.<Number>>>
        //
        // The second Regular Expression will clean up the more complex expression where there
        // may be an either or such as Promise.<(FOO | BAR), BAZ)> signature. Once again the
        // period between the Promise and < will be removed.
        // Promise.<(module:esri/geometry/Point|module:esri/Graphic), Error>
        // --------------------------------------------------------------------
        var standardADTRegEx = /\.&lt;\(?<[^>]*>[a-zA-Z0-9:\\/]*<[^>]*>>/i;
        var complexADTRegEx = /\.&lt;\(?(<[^>]*>[a-zA-Z0-9:\\/\|]*<[^>]*>[,\s\\|\)]*)*/i;
        if (standardADTRegEx.test(stringifiedType) || complexADTRegEx.test(stringifiedType)){
            // --------------------------------------------------------------------
            // Cleanup @return {Promise<TypeDef>}
            // @return {Promise<module:esri/renderers/smartMapping/creators/color~PCTrueColorRendererResult>}
            // <a href="../guide/working-with-promises/">Promise</a>.&lt;<a href="module-esri-renderers-smartMapping-creators-color.html#PCTrueColorRendererResult">color~PCTrueColorRendererResult</a>>
            // From Promise<color~PCTrueColorRendererResult> to Promise<PCTrueColorRendererResult>
            // --------------------------------------------------------------------
            if (/\w*~\w*<\/a>>/i.test(stringifiedType)) {
                var typeDefReturnType = stringifiedType.replace(/\w*~/gi, "");
                var cleansedStringifiedType = typeDefReturnType.replace(/\.&lt;/g, "&lt;");
                stringifiedType = cleansedStringifiedType;
            }
            else {
                var cleansedStringifiedType = stringifiedType.replace(/\.&lt;/g, "&lt;");
                stringifiedType = cleansedStringifiedType;
            }
        }
        else {
    ...
    else if (/\.&lt;<[^>]*>[a-zA-Z0-9:\\/]*<[^>]*>>/i.test(stringifiedType)){
        var cleansedStringifiedType = stringifiedType.replace(/\.&lt;/g, "&lt;");
        stringifiedType = cleansedStringifiedType;
    }
...
function buildLink(longname, linkText, options) {
    ...
    // Promise.<Array.<Array.<Array.<Array.<Number>>>>>
    //
    // Stringified by Catharsis
    //
    // <a href="../guide/working-with-promises/">Promise</a>.<Array.<<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number">Number</a>>>
    // <a href="../guide/working-with-promises/">Promise</a>.<Array.<Array.<<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number">Number</a>>>>
    // <a href="../guide/working-with-promises/">Promise</a>.<Array.<Array.<Array.<<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number">Number</a>>>>>
    // <a href="../guide/working-with-promises/">Promise</a>.<Array.<Array.<Array.<Array.<<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number">Number</a>>>>>>
    //
    // <a href="../guide/working-with-promises/">Promise</a>.&lt;Array.&lt;<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number">Number</a>>>
    // <a href="../guide/working-with-promises/">Promise</a>.&lt;Array.&lt;Array.&lt;<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number">Number</a>>>>
    // <a href="../guide/working-with-promises/">Promise</a>.&lt;Array.&lt;Array.&lt;Array.&lt;<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number">Number</a>>>>>
        ...
        if ( longname.toLowerCase() === 'object.<string, string>') {
            ...
            if(matchArray && matchTypeArray){
              ...
              if (arrayPrefixType.match(matchLowerCaseWordRegEx)){
                ...
                // --------------------------------------------------------------------
                // Promise.<Array.<module:esri/Graphic>>
                // --------------------------------------------------------------------
                var stringifiedParsedType = stringifyType(parsedType, options.cssClass, options.linkMap);
                ...
                // /(Array\.?(<|&lt;))+
                // --------------------------------------------------------------------
                var numberOfArraysRegEx = /Array\.?(<|&lt;)/g;

                // --------------------------------------------------------------------
                // Regular Expression to match the full inner signature of a Promise
                // returning an array of some data type.
                // e.g., Promise<Graphic[]>
                // Used to remove the inner part of the stringified parsed type
                // <a href="../guide/working-with-promises/">Promise</a>.<FOOBARBAZ>
                // <a href="../guide/working-with-promises/">Promise</a>.&lt;FOOBARBAZ>
                // FOOBARBAZ would be removed from the above segment
                //
                // Raw regex: (Array\.?(<|&lt;))+<[^>]*>[-~:/\w]+<[^>]*>>{1}
                // --------------------------------------------------------------------
                var innerArrayRegExRaw =  "(Array\\.?(<|&lt;))+<[^>]*>[-~:\\w\/]+<[^>]*>>{COUNT}";

                if (numberOfArraysRegEx.test(stringifiedParsedType)) {
                    var numberOfMatches = stringifiedParsedType.match(numberOfArraysRegEx).length;
                    // complete the Regular Expression by replacing COUNT with the real number
                    var innerArrayRegEx = RegExp(innerArrayRegExRaw.replace("COUNT", numberOfMatches));
                    // capture the single or multi-dimensional array including the html anchor
                    var singleToMultiDimArray = stringifiedParsedType.match(innerArrayRegEx);
                    if (singleToMultiDimArray){
                        // captured the opening and closing element of the anchor tag
                        var anchorPartsMatch = singleToMultiDimArray[0].match(/<[^>]*>/g);
                        // single: [], or multi: [][][] depending on the complexity
                        var arrayBracketSignature = stringifiedParsedType.match(numberOfArraysRegEx).map(() => { return "[]"});
                        // the completed html anchor <a href="foo">Bar[]</a> link
                        // instead of Array.<<a href="foo">Bar</a>> link
                        var innerAnchor = anchorPartsMatch[0] + arrayPrefixSignature + arrayBracketSignature.join("") + anchorPartsMatch[1];

                        return stringifiedParsedType.replace(innerArrayRegEx, innerAnchor);
                    }
                    else {
