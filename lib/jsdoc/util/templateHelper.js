...
function getFilename(longname) {
    ...
    // --------------------------------------------------------------------
    const hasKey = Object.keys(longnameToUrl).indexOf(longname) !== -1;
...
const stringifyType = exports.stringifyType = (parsedType, cssClass, stringifyLinkMap) => {
    ...
    // --------------------------------------------------------------------
    let stringifiedType = require('catharsis').stringifyEsri(parsedType, {
        cssClass: cssClass,
        htmlSafe: true,
...
exports.getSignatureReturns = ({yields, returns}, cssClass) => {
    ...
    if (returnTypes && returnTypes.length) {
        returnTypes = returnTypes.map(r => linkto(r, htmlsafe(parseEsriString("name", r)), cssClass));
    }
