...
// --------------------------------------------------------------------
// Load the Esri configuration variables
// --------------------------------------------------------------------
const config = require("../../../../../libs/config-esri").load();
const JSAPI_TARGET = config.DEFAULTS.JSAPI_TARGET;
const JSAPI_TARGET_KEY = JSAPI_TARGET.toUpperCase();
const JSAPI_ARCGIS_JS_API_URL = config[JSAPI_TARGET_KEY].JSAPI_ARCGIS_JS_API_URL;
  ...
  var markdown = new MarkdownIt({
    ...
    highlight: function (code, lang){
      if (code.indexOf("@link") !== -1) {
        ...
        return (lang) ? hljs.highlight(code, { language: lang, ignoreIllegals: true }).value : hljs.highlight(code, { language: "js", ignoreIllegals: true }).value;
      }
    }
  });

    markdown.use(markdownItContainer, "esri-md-deprecation-notice", {
    validate: function (params){
      //console.log("validate params (%s)", params);
      var tagMatch = params.trim().match(/^esri-md-deprecation-notice\s*(.*)$/);
    ...
    render: function (tokens, idx){
      ...
      if (tokens[idx].nesting === 1) {
        // opening tag
        return `<div style="max-width:100%;text-align:center;padding: 1.5rem 0;">
 <calcite-notice icon="" active="" scale="l" width="full" color="red">`;
      
      }
      else {
        // closing tag
        return "</calcite-notice></div>";
      }
    }
  });
  // --------------------------------------------------------------------
  ...
  markdown.use(markdownItContainer, "esri-md", {
    ...
    render: function (tokens, idx){
      ...
      if (tokens[idx].nesting === 1) {
        // opening tag
        if (cssClass !== "") {
          return `<div class="esri-md ${cssClass.trim()}" style="max-width:100%;text-align:center;">
  <calcite-notice active="" scale="l" width="auto" color="red">
    <div slot="message">`;
        }
        else {
          return `<div class="esri-md" style="max-width:100%;text-align:center;">
  <calcite-notice active="" scale="l" width="auto" color="red">
    <div slot="message">`;
        }
      }
        ...
        // closing tag
        return '</div></calcite-notice></div>\n';
      }
    }
  });
  // --------------------------------------------------------------------
  // Acetate uses the markdown-it parser to translate *.md files into
  // html content. The parser is exposed through the markdown instance,
  // loading the markdown-it-container plugin allows us to decorate block
  // content in our *.md files and assign calcite-web or other selectors
  // to the content container (div) tag. This provides a mechanism to
  // still write in markdown, but style the content using calcite-web
  // styles or write our own.
  //
  ...
  // Output:
  //
  // <div class="esri-read-more accordion js-accordion" aria-live="polite" role="tablist">
  //   <div class="accordion-section" aria-expanded="true">
  //     <h4 class="accordion-title" role="tab" tabindex="0">Read more</h4>
  ...
  // </ul>
  //
  // <div class="panel panel-light-blue"><p>markdown-it-container rulezz!
  // It is amazing</p>
  //
  // </div>
  // <blockquote>
  ...
  // </div>
  //
  // https://github.com/patrickarlt/acetate/blob/master/lib/mixins/helpers.js
  // https://github.com/patrickarlt/acetate/blob/master/lib/mixins/markdown.js
  // --------------------------------------------------------------------
  markdown.use(markdownItContainer, "esri-read-more",
    {
      marker: ">",
      validate: function(params) {
        return params.trim().match(/^esri-read-more/);
      },
      render: function(tokens, idx) {
        // console.log(`tokens: (${JSON.stringify(tokens[idx], undefined, 2)}), idx: ${idx}`);

        // opening tag
        if (/container_esri-read-more_open/i.test(tokens[idx].type)) {
          return `<div style="background-color:#f8f8f8;border-left:solid 3px #c8c8c8;padding:0.3125rem"><calcite-accordion>
        <calcite-accordion-item item-title="Read more" selection-mode="single">\n`;
        }
        // closing tag
        if (/container_esri-read-more_close/i.test(tokens[idx].type)) {
          return `  </calcite-accordion-item>
  ...
  if (parserName === parserNames.markdownit) {
    ...
    parserFunction = function (source){
      ...
      // --------------------------------------------------------------------
      source= source.replace(/{+\s+JSAPI_ARCGIS_JS_API_URL\s+}+/g, JSAPI_ARCGIS_JS_API_URL);
