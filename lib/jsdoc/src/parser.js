...
Parser.prototype._parseSourceCode = function(sourceCode, sourceName) {
    ...
    this.emit('fileBegin', e);
		// --------------------------------------------------------------------
		// If the file matches an Esri path such as arcgis-js-api-4 or
		// arccgis-js-api, then remove the /Users/foo/bar/baz/arcgis-js-api-4
		// --------------------------------------------------------------------
		var removeEsriJSDocPathRegEx = /.*(?=arcgis[_-]js[_-]api[_-]?\d)arcgis[_-]js[_-]api[_-]?\d/im;
		if(sourceName.match(removeEsriJSDocPathRegEx)) {
				logger.info('Parsing %s', sourceName.replace(removeEsriJSDocPathRegEx, ""));
		}
		else {
