...
function cleanseStringify(parsedType, stringifiedValue){
  ...
  if (/>(object|function|number|string|boolean)<\/a>/g.test(stringifiedValue)) {
    // find the match
      var typeRegEx = /\w*(?=<\/a>)/i;
      var linkTextMatches = stringifiedValue.match(typeRegEx);
      linkTextMatches.forEach((linkTextMatch) => {
        var linkTextFixed = linkTextMatch[0].toUpperCase() + linkTextMatch.substring(1);
        var linkTextRegEx = new RegExp(linkTextMatch, "g");
...
var stringifyType = exports.stringifyType = function(parsedType, cssClass, stringifyLinkMap) {
    ...
    // --------------------------------------------------------------------
    var stringifiedType = require('catharsis').stringifyEsri(parsedType, {
        cssClass: cssClass,
        htmlSafe: true,
