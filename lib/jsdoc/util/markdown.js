...
function getParseFunction(parserName, conf) {
    ...
    // --------------------------------------------------------------------
    // load the highlight.js module which will be used in the 
		// marked.setOptions section below
    // --------------------------------------------------------------------
	  ...
	  // Synchronous highlighting with highlight.js
		//
		// The original markedRenderer.code setup is commented out below.
	  // --------------------------------------------------------------------
	  marked.setOptions({
			highlight: function (code, lang){
				// --------------------------------------------------------------------
				// Just return the code if there is a JSDoc link in the source code
				// --------------------------------------------------------------------
	      if (code.indexOf("@link") !== -1) {
	        return code;
	      }
	      else {
					// if a language wasn't passed in highlight using JavaScript
    ...
    if (parserName === parserNames.marked) {
        ...
        // Allow prettyprint to work on inline code samples
				//
				// This section was commented out in favor of using highlight.js
				// --------------------------------------------------------------------
				/*
        markedRenderer.code = function(code, language) {
            ...
            return util.format( '<pre class="prettyprint source%s"><code>%s</code></pre>',
				...
				*/
				markedRenderer.codespan = function (code, lang){
					// --------------------------------------------------------------------
					// Just return the code if there is a JSDoc link in the source code
					// --------------------------------------------------------------------
		      if (code.indexOf("@link") !== -1) {
		        return code;
						...
						code = code.replace(/&quot;/g, '"');
						// if a language wasn't passed in highlight using JavaScript
						var highlightedCode = (lang) ? hljs.highlight(lang, code).value : hljs.highlight("js", code).value;
